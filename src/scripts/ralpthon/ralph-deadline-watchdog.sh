...
# Capture start_time once at launch (does not change in loop)
start_time=$(date +%s)
...
while true; do
  ...
  # Question Mode: Auto-accept permission prompts
  if echo "$current_content" | grep -qiE "permission|allow|Permanent|Confirm|do you want|abort|denied|yes/no|y/N|y/n"; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S %Z')] Permission prompt detected. Auto-accepting (y)..." >> "$WATCHDOG_LOG"
  ...
  if [ $inactivity -ge $INACTIVITY_THRESHOLD ]; then
    ...
    echo "[$(date '+%Y-%m-%d %H:%M:%S %Z')] No activity for ${inactivity}s. Sending continue prompt (${remaining}s remaining)." >> "$WATCHDOG_LOG"
