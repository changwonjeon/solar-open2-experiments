...
> Schema and format definitions for `/solar-ralph`'s persistent state files. All state files are located in `data/results/ralphthon/solar/<run-id>/`, where `<run-id>` is generated at `start` time as `solar-ralph-<YYYYMMDD-HHMMSS>`.
...
- Does not include files in `_private/`, `data/results/ralphthon/solar/<run-id>/` itself, or other ignored/excluded paths.
...
`data/results/ralphthon/solar/<run-id>/HANDOFF.md`
