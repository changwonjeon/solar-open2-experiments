...
def create_navigator_agent(model_name: str = "google_genai:gemini-2.5-pro", model_provider: str | None = None, temperature: float = 0.1):
    """구조화된 정보 수집 및 탐색용 에이전트 생성"""
    if model_provider:
        nav_model = init_chat_model(model_name, model_provider=model_provider, temperature=temperature)
    else:
