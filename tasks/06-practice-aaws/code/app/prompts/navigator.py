...
### 종료 및 도구 예산
- 단순 목록·필터 시나리오는 `get_page_structure`와
  `verify_selectors_with_samples`를 중심으로 필요한 최소 호출만 사용하세요.
- 핵심 셀렉터와 필터/페이지네이션 방식이 검증되면 추가 관찰을 반복하지 말고
  즉시 Blueprint를 반환하세요.
- 같은 목적의 도구 호출은 최대 2회까지만 허용하며, 두 번째에도 확인되지 않으면
  불확실성을 `navigator_notes`에 기록하고 종료하세요.
"""
