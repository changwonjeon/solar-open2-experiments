...
def _get_active_model_config() -> tuple[str, str | None]:
    """현재 실행 모드에 따라 모델명/프로바이더를 반환합니다."""
    mode = os.getenv("AAWS_MODEL_MODE", "gemini")
    if mode == "upstage":
        return "solar-open2", "upstage"
    return "google_genai:gemini-2.5-flash", None
...
async def get_page_structure(url: str, scraping_goal: str, wait_seconds: float = 2.0) -> str:
    ...
    model_name, model_provider = _get_active_model_config()
    analysis_llm = init_chat_model(model_name, model_provider=model_provider, temperature=0)
...
async def browse_web(runtime: ToolRuntime[NavigatorContext], url: str, instruction: str, purpose: str = "", context: str = "") -> str:
    ...
    try:
        ...
        return f"[Error] 브라우저 에이전트 실행 실패: {e}"
    finally:
        if tb is not None:
            await tb.stop()
