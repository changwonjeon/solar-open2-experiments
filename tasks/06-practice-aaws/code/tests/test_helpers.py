...
def setup_scenario_context(scenario_file: str, project_root: str, prefix: str, suffix: str = ""):
    """
    시나리오 파일 및 결과/로그 출력 경로를 초기화합니다.
    suffix가 있으면 파일명 끝에 '_upstage'처럼 붙여서 결과를 분리 저장합니다.
    """
    ...
    json_output_path = os.path.join(scenario_out_dir, f"{prefix}_result{suffix_text}.json")
    log_output_path = os.path.join(scenario_out_dir, f"{prefix}_log{suffix_text}.md")

    with open(log_output_path, "w", encoding="utf-8") as log_file:
        log_file.write(f"# 시나리오 실행 로그 ({prefix}{suffix_text}): {scenario.scenario_id}\n\n")
